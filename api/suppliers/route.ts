...
export async function POST(req: NextRequest) {
    try {
        ...
        const { name, email, phone, address, city, state, zipCode, taxNumber, company, notes } = body;
        ...
        const supplier = {
            ...
            company: company || '', notes: notes || '', status: 'active' as const, dueAmount: 0,
            createdAt: new Date(), updatedAt: new Date(),
        };

        const result = await collection.insertOne(supplier as any);
        return NextResponse.json({ _id: result.insertedId.toString(), ...supplier }, { status: 201 });
    } catch (error) {
